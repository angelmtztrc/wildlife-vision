...
def test_run_delegates_steps_in_order_and_aggregates_results(
    ...
    leftover = initial_path / "leftover.txt"
    ...
    assert corrupted_input.source == initial_path
    assert corrupted_input.output == session_path
    assert corrupted_input.dry_run is True

    assert overexposed_input.source == initial_path
    ...
    assert bursts_input.source == initial_path
    ...
    assert detect_input.source == initial_path
...
def test_run_accepts_mixed_case_camera_segment_and_aggregates_failures(
    ...
    assert result.files_remaining_in_initial == 0
