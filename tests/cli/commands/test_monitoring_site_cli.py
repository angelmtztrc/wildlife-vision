...
def test_monitoring_site_cli_requires_geography_and_area(
    ...
    area = cli_runner.invoke(app, ["monitoring-area", "create", "--id", "AREA002", "--name", "South Ranch"])
    created = cli_runner.invoke(
        app,
        [
            "monitoring-site", "create", "--id", "SITE003", "--area", "AREA002",
            "--name", "Fence Trail", "--latitude", "28.57", "--longitude", "-101.16",
        ],
    )
    listed = cli_runner.invoke(
        app, ["monitoring-site", "list", "--area", "AREA002"], terminal_width=120
    )
    shown = cli_runner.invoke(app, ["monitoring-site", "show", "SITE003"])

    assert area.exit_code == 0
    assert created.exit_code == 0
    assert "SITE ID" in listed.output
    assert "AREA ID" in listed.output
    assert "NAME" in listed.output
    assert "SITE003" in listed.output
    assert "AREA002" in listed.output
    assert "Fence Trail" in listed.output
...
def test_monitoring_site_cli_rejects_missing_coordinates(
    ...
    result = cli_runner.invoke(
        app, ["monitoring-site", "create", "--area", "AREA001", "--name", "Missing"]
    )
