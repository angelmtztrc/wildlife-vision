...
class ResearchGradeApp:
    ...
    def _build_layout(self) -> None:
        ...
        tk.Button(footer, text="Skip", command=self._skip_and_refresh).pack(
            side="left", padx=(0, 6)
        )
        tk.Button(footer, text="Next", command=self._next_and_refresh).pack(
            side="left", padx=(0, 6)
        )
        tk.Button(footer, text="Zoom +", command=self._zoom_in_and_refresh).pack(
            side="left", padx=(12, 6)
        )
        tk.Button(footer, text="Zoom -", command=self._zoom_out_and_refresh).pack(
            side="left", padx=(0, 6)
        )
        tk.Button(
            footer, text="Reset Zoom", command=self._reset_zoom_and_refresh
        ).pack(side="left", padx=(0, 6))
        tk.Label(footer, textvariable=self.zoom_var).pack(side="right", padx=(6, 0))
        tk.Button(footer, text="Save & Exit", command=self.save_and_exit).pack(
            side="right"
        )
    ...
    def refresh(self) -> None:
        ...
        if item is None:
            ...
            self.detail_var.set(
                "The animal detection bucket has no supported images matching the current filter."
            )
    ...
    def save_and_exit(self) -> None:
        ...
        if result.files_failed > 0:
            messagebox.showerror("Save failed", self._format_commit_result(result))
            self.refresh()
            return
