...
def register_bindings(root, controller: ReviewController, refresh: Callable[[], None], on_save: Callable[[], None], on_close: Callable[[], None]) -> None:
    ...
    root.bind("4", lambda event: _assign(controller, "domestic", refresh))
    root.bind("5", lambda event: _assign(controller, "empty", refresh))
    root.bind("6", lambda event: _assign(controller, "other", refresh))
